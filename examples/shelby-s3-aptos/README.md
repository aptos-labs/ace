...
2. During upload, the owner also mints an access token by generating a fresh Ed25519 keypair.
...
```text
token private key -> signs ACE request -> token address -> allowlist hit -> decrypt
```
