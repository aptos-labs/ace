...
/// derives a deterministic BLS keypair `(access_private_key, access_public_key)` from `(owner, blob_id)`
/// via ACE's threshold VRF, and registers `access_public_key` on-chain here. The owner
/// shares `access_private_key` out-of-band as a single-token access grant ("pre-signed
/// URL" semantics): anyone who holds `access_private_key` can ask the ACE workers to
