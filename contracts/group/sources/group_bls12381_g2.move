...
/// BLS12-381 G2 group + Fr scalar field. Mirror of `ace::group_bls12381_g1`.
///
/// Note: Fr is shared between G1 and G2 (both groups have the same prime order),
/// so `PrivateScalar` semantics here are identical to the G1 module's. We keep
/// them as separate types so the abstract `ace::group::Scalar` enum can faithfully
/// carry a scheme tag and prevent accidental cross-group operations.
module ace::group_bls12381_g2 {
    use aptos_std::aptos_hash;
    use aptos_std::bcs_stream::{Self, BCSStream};
    use aptos_std::crypto_algebra::{Self, Element};
    use aptos_std::bls12381_algebra::{G2, FormatG2Compr, Fr, FormatFrLsb, HashG2XmdSha256SswuRo};
    ...
    const G2_COMPRESSED_BYTES: u64 = 96;
    const FR_SCALAR_BYTES: u64 = 32;
    const DST: vector<u8> = b"ace::group_bls12381_g2";
    ...
    public fun identity(): PublicPoint {
        from_inner_element(&crypto_algebra::zero<G2>())
    }
    
    public fun point_sum(pks: &vector<PublicPoint>): PublicPoint {
        let accumulator = crypto_algebra::zero<G2>();
        pks.for_each_ref(|pk| {
    ...
    public fun msm(points: vector<PublicPoint>, scalars: vector<PrivateScalar>): PublicPoint {
        let point = crypto_algebra::multi_scalar_mul<G2, Fr>(
    ...
    public fun deserialize_public_point(stream: &mut BCSStream): PublicPoint {
        let point = deserialize_g2_point(stream);
        from_inner_element(&point)
    }

    #[lint::allow_unsafe_randomness]
    public fun rand_element(): PublicPoint {
        from_inner_element(&crypto_algebra::hash_to<G2, HashG2XmdSha256SswuRo>(&DST, &randomness::bytes(32)))
    }

    public fun element_from_hash(msg: &vector<u8>): PublicPoint {
        from_inner_element(&crypto_algebra::hash_to<G2, HashG2XmdSha256SswuRo>(&DST, msg))
    }

    /// Hash arbitrary bytes to a BLS12-381 Fr scalar via SHA2-512.
    /// Identical to `group_bls12381_g1::hash_to_scalar` (same Fr, same algorithm).
    ...
    fun to_inner_element(element: &PublicPoint): Element<G2> {
        crypto_algebra::deserialize<G2, FormatG2Compr>(&element.point).destroy_some()
    }
    ...
    fun from_inner_element(element: &Element<G2>): PublicPoint {
        PublicPoint { point: crypto_algebra::serialize<G2, FormatG2Compr>(element) }
    }
    ...
    fun deserialize_g2_point(stream: &mut BCSStream): Element<G2> {
        let bytes = deserialize_bytes_field(stream);
        let opt = crypto_algebra::deserialize<G2, FormatG2Compr>(&bytes);
        opt.destroy_some()
    }
