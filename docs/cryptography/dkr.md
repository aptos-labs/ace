...
Properties 1–4 below follow from the corresponding per-VSS property ([`vss.md`](./vss.md) §2) composed by the Lagrange-at-zero combination of §1.1. They assume **static corruption with the PSS budget**: an adversary corrupts at most $b_\text{old} < t$ nodes in the old committee and at most $b_\text{new} < t'$ in the new committee.

**1. MSK preservation (correctness).** DKR outputs a new committee whose joint Shamir share interpolates to the same master secret $s$ as the old committee's. — Immediate from §1.1: $F(x) = \sum_{j \in H} \lambda_j g_j(x)$ has degree $t'-1$, $F(0) = \sum_j \lambda_j s_j = f(0) = s$, and each new node $i$'s share is $F(i+1)$.

**2. Completeness.** Up to $t-1$ malicious old dealers cannot prevent honest new-committee output. — At least $n_\text{old} - (t-1) \geq t$ honest old dealers succeed in their VSS (per-VSS completeness), so $|H|$ reaches threshold from honest dealers alone, and Lagrange-at-zero recovers $s$.

**3. Termination.** All honest new-committee nodes terminate within $O(\Delta)$ under synchrony + L1 liveness. — Per-VSS termination for each contributing old dealer; on-chain $H$-freezing and Lagrange-coefficient computation add only finite deterministic computation.

**4. Secrecy across the transition window.** The adversary's view of the DKR transcript is computationally simulatable from $(\mathsf{masterPk}, \{s_j : j \in J_\text{old}\}, \{S_i : i \in J_\text{new}\})$ — i.e., from what the parent committee already published plus the corrupted parties' own old and new shares. In particular, the adversary learns nothing about the master secret $s = \log_g \mathsf{masterPk}$ beyond DLog hardness, and nothing about honest new shares beyond what the corrupted shares reveal.
