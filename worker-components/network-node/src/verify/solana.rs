...
//! Solana proof-of-permission verification — basic and custom flows.
//!
//! For both flows the on-the-wire payload is a serialized Solana transaction
//! (legacy or v0-versioned). We:
//!   1. Parse it structurally and check the program ID + instruction layout
//!      matches what the client would build for this request (the inline
//!      `full_request_bytes` / `CustomFullRequestBytes` blob).
//!   2. Submit it to the configured Solana RPC via `simulateTransaction`
//!      with `sigVerify: true` and `commitment: confirmed` — the RPC then
//!      checks the signatures, the recent blockhash, and runs the program.
...
fn validate_txn(
    ...
    let (account_keys, instructions) = if is_versioned {
        parse_txn_versioned(txn)?
    } else {
        parse_txn_legacy(txn)?
    };
...
fn parse_txn_legacy(bytes: &[u8]) -> Result<(Vec<[u8; 32]>, Vec<SolanaInstruction>)> {
    let mut pos = 0usize;
...
fn parse_txn_versioned(bytes: &[u8]) -> Result<(Vec<[u8; 32]>, Vec<SolanaInstruction>)> {
    let mut pos = 0usize;
...
fn read_pubkeys(bytes: &[u8], pos: &mut usize, count: usize) -> Result<Vec<[u8; 32]>> {
...
fn read_instructions(bytes: &[u8], pos: &mut usize) -> Result<Vec<SolanaInstruction>> {
