...
//! `epoch-change-cur` binary — thin CLI over [`epoch_change_cur::run`].

use clap::{Parser, Subcommand};
use tokio::sync::oneshot;

#[derive(Parser)]
#[command(name = "epoch-change-cur", version, about)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
#[derive(Subcommand)]
enum Commands {
    /// Run the current-committee epoch-change client for one `epoch_change::Session` until it reaches `STATE__DONE`.
    Run(RunArgs),
}
...
async fn main() {
    ...
    match cli.command {
        Commands::Run(args) => {
            ...
            if let Err(e) = epoch_change_cur::run(cfg, shutdown_rx).await {
                eprintln!("epoch-change-cur: fatal: {:#}", e);
                std::process::exit(1);
            }
        }
    }
