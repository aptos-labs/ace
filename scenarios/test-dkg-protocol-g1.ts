...
/**
 * DKG protocol e2e — regression coverage for BLS12-381 G1 (scheme = 0).
 *
...
async function main() {
    ...
    try {
        ...
        const tmpRoot = mkdtempSync(path.join(tmpdir(), 'ace-dkg-g1-'));
        ...
        try {
            ...
            if (session.scheme !== ace.vss.SCHEME_BLS12381G1) {
                throw `expected DKG scheme = ${ace.vss.SCHEME_BLS12381G1}, got ${session.scheme}`;
            }

            log(`DKG complete. aggregate C0: ${session.commitmentPoints[0].toHex()}`);
            ...
            for (let j = 0; j < numWorkers; j++) {
                ...
                if (!session.basePoint.scale(combinedOpenings[j].p).equals(session.sharePks[j])) {
            ...
            if (!computedC0.equals(session.commitmentPoints[0])) {
                throw 'Reconstructed secret/blinding does not match DKG aggregate C0.';
            }
            if (!session.basePoint.scale(reconstructedSecret).equals(session.resultPk!)) {
                throw 'Reconstructed secret does not match DKG result PK.';
            }
            log(`DKG correctness verified. aggregate C0: ${session.commitmentPoints[0].toHex()}`);
        } finally {
