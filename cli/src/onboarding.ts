...
export function gceDeployCmd(
    ...
    const script = [
        ...
        ...startupTail,
        ``,
        `STATIC_IP="$(gcloud compute addresses describe "$STATIC_IP_NAME" --project "$PROJECT" --region "$REGION" --format='value(address)' 2>/dev/null || true)"`,
        `if [ -z "$STATIC_IP" ]; then`,
        `  ADDR_CREATE_LOG="$(mktemp)"`,
        `  if ! gcloud compute addresses create "$STATIC_IP_NAME" --project "$PROJECT" --region "$REGION" >"$ADDR_CREATE_LOG" 2>&1; then`,
        `    STATIC_IP="$(gcloud compute addresses describe "$STATIC_IP_NAME" --project "$PROJECT" --region "$REGION" --format='value(address)' 2>/dev/null || true)"`,
        `    if [ -z "$STATIC_IP" ]; then`,
        `      cat "$ADDR_CREATE_LOG" >&2`,
        `      rm -f "$ADDR_CREATE_LOG"`,
        `      exit 1`,
        `    fi`,
        `  else`,
