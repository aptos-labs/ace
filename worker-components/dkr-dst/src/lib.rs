...
pub async fn run(config: RunConfig, mut shutdown_rx: oneshot::Receiver<()>) -> Result<()> {
    ...
    let my_dst_idx = initial
        .new_nodes
        .iter()
        .position(|n| *n == account_addr)
        .ok_or_else(|| anyhow!("account {} not found in DKR new_nodes", account_addr))?;
    ...
    loop {
        ...
        if session.state_code >= STATE_FAIL {
            ...
            return Err(anyhow!(
                "dkr-dst: DKR session failed (state_code={})",
                session.state_code
            ));
        }
        ...
        let should_touch = match session.state_code {
            ...
            STATE_VSS_IN_PROGRESS => {
                should_submit_rotating_touch(my_dst_idx, session.new_nodes.len())
            }
            _ => true,
        };
        if should_touch {
        ...
        println!(
            "dkr-dst: DKR in progress (state_code={}, vss_sessions={})",
            session.state_code,
