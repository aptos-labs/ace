...
/**
 * Step 2 — Alice deploys `presigned_access` under her own address and
...
async function main() {
    ...
    log(`Deploying presigned_access with admin = ${adminAddress}`);
    ...
    const txn = await aptos.transaction.build.simple({
        ...
        data: {
            function: `${adminAddress}::presigned_access::init` as `${string}::${string}::${string}`,
            typeArguments: [],
            functionArguments: [],
        },
    });
