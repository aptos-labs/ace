...
**4. Secrecy across the transition window.** The adversary's view of the DKR transcript is computationally simulatable from $(s, \{s_j : j \in J_\text{old}\}, \{S_i : i \in J_\text{new}\})$ — i.e., it learns nothing about other parties' new shares beyond what its corrupted old shares + corrupted new shares already reveal.
