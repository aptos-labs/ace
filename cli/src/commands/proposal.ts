...
function generateTemplate(state: aceNetwork.State): string {
    ...
    return [
        ...
        state.secrets.length > 0
            ...
            : `secrets_to_retain = []`,
        ``,
        `# new_secrets: generate one fresh DKG keypair for each listed primitive id.`,
        `# Supported primitive ids:`,
        ...primitiveCatalogLines(),
        `# Example: \`new_secrets = [1]\` creates a BFIBE short-signature AEAD keypair.`,
        `# Example: \`new_secrets = [2]\` creates a BLS12381 threshold VRF keypair.`,
        `# A request may list multiple same-group primitives for one shared keypair:`,
        `#   new_secrets = [{ primitives = [1, 2], note = "BFIBE shortsig + VRF" }]`,
