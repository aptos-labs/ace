...
/**
 * Shamir secret sharing over BLS12-381 Fr with curve base in **G2**.
 * Secret is `(B, s)` with `B` a random G2 point and `s` ∈ Fr; public commitment is `(B, s·B)` in G2.
 */
...
const G2_COMPRESSED_BYTES = 96;

/** G2 `Secret` BCS: `version: u8`, `scalar: [u8;32]` (Fr LE), `B: [u8;96]` compressed G2. */
export class Secret {
    constructor(
    ...
    static deserialize(deserializer: Deserializer): Result<Secret> {
        return Result.capture({
            ...
            task: () => {
                ...
                const Bbytes = deserializer.deserializeFixedBytes(G2_COMPRESSED_BYTES);
                const B = bls12_381.G2.Point.fromBytes(Bbytes) as unknown as WeierstrassPoint<bigint>;
    ...
    static fromBytes(bytes: Uint8Array): Result<Secret> {
        return Result.capture({
            ...
            task: () => {
                ...
                const secret = Secret.deserialize(deserializer).unwrapOrThrow("Bls12381G2.Secret.fromBytes deserialize");
    ...
    static fromHex(hex: string): Result<Secret> {
        return Result.capture({
            recordsExecutionTimeMs: false,
            task: () => Secret.fromBytes(hexToBytes(hex)).unwrapOrThrow("Bls12381G2.Secret.fromHex failed"),
        });
    }
}

/** `(B, s·B)` BCS: `version: u8`, `B: [u8;96]`, `sB: [u8;96]` compressed G2. */
export class PublicCommitment {
    ...
    static deserialize(deserializer: Deserializer): Result<PublicCommitment> {
        return Result.capture({
            ...
            task: () => {
                ...
                const Bbytes = deserializer.deserializeFixedBytes(G2_COMPRESSED_BYTES);
                const sBbytes = deserializer.deserializeFixedBytes(G2_COMPRESSED_BYTES);
                const B = bls12_381.G2.Point.fromBytes(Bbytes) as unknown as WeierstrassPoint<bigint>;
                const sB = bls12_381.G2.Point.fromBytes(sBbytes) as unknown as WeierstrassPoint<bigint>;
        ...
        return Result.capture({
            ...
            task: () => {
                ...
                const pc = PublicCommitment.deserialize(deserializer).unwrapOrThrow(
                    "Bls12381G2.PublicCommitment.fromBytes deserialize",
                );
    ...
    static fromHex(hex: string): Result<PublicCommitment> {
        return Result.capture({
            recordsExecutionTimeMs: false,
            task: () => PublicCommitment.fromBytes(hexToBytes(hex)).unwrapOrThrow("Bls12381G2.PublicCommitment.fromHex failed"),
        });
    }
}
...
/**
 * G2 `SecretShare` BCS:
 * `version: u8`, `n: u64`, `t: u64`, `seed: [u8;32]`, `x: [u8;32]`, `y: [u8;32]`, `B: [u8;96]` compressed G2.
 * In memory, `base` is the decoded G2 point (same on every share from one `split`).
 */
export class SecretShare {
    constructor(
        ...
        readonly x: bigint,
        readonly y: bigint,
        /** G2 base `B` (same on every share from one `split`). */
        readonly base: WeierstrassPoint<bigint>,
    ) {
    ...
    static deserialize(deserializer: Deserializer): Result<SecretShare> {
        return Result.capture({
            ...
            task: () => {
                ...
                const baseBytes = deserializer.deserializeFixedBytes(G2_COMPRESSED_BYTES);
                const B = bls12_381.G2.Point.fromBytes(baseBytes) as unknown as WeierstrassPoint<bigint>;
    ...
    static fromBytes(bytes: Uint8Array): Result<SecretShare> {
        return Result.capture({
            ...
            task: () => {
                const deserializer = new Deserializer(bytes);
                const share = SecretShare.deserialize(deserializer).unwrapOrThrow("Bls12381G2.SecretShare.fromBytes deserialize");
    ...
    static fromHex(hex: string): Result<SecretShare> {
        return Result.capture({
            recordsExecutionTimeMs: false,
            task: () => SecretShare.fromBytes(hexToBytes(hex)).unwrapOrThrow("Bls12381G2.SecretShare.fromHex failed"),
        });
    }
}

export function shareWireLengthG2(): number {
    return 1 + 8 + 8 + SSS_SEED_BYTES + 32 + 32 + G2_COMPRESSED_BYTES;
}
...
export function keygen(): { secret: Secret; publicCommitment: PublicCommitment } {
    const B = bls12_381.G2.hashToCurve(randomBytes(32)) as unknown as WeierstrassPoint<bigint>;
