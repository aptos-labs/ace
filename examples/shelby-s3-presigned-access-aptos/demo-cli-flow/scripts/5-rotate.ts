...
/**
 * Step 5 — Alice rotates the `accessPk` on-chain.
 *
 * Re-registering under the same `blob_suffix` overwrites the previous
 * entry; the old `accessToken` no longer signs anything that verifies
 * under the new `accessPk`. This is the demo's "revoke" knob.
 *
 * The rotation does NOT have to go through tVRF — the contract only
 * checks that `accessPk` is a well-formed G1 point. Alice could re-derive
...
async function main() {
    ...
    const txn = await aptos.transaction.build.simple({
        ...
        data: {
            ...
            typeArguments: [],
            functionArguments: [BLOB_SUFFIX, accessPkPrime],
        },
    });
    ...
    log('Rotation done. The accessToken in data/grant.json is now stale.');
    log('');
